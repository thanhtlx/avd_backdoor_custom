...
if __name__ == "__main__":
  ...
  for split in ["test","train","valid"]:
    if not os.path.isfile('datasets/normalized/csn/{}.jsonl.gz'.format(split)):
        continue
    if split == 'baseline':
      has_baselines = True
    get_site_map = False
    if os.path.exists('datasets/normalized/csn/{}_site_map.json'.format(split)): 
      with open('datasets/normalized/csn/{}_site_map.json'.format(split), 'r') as f:
    ...
    if get_site_map:
      with open('datasets/outputs/{}_site_map.json'.format(split), 'w') as f:
          json.dump(new_site_map, f)
  
  pool = multiprocessing.Pool()
  print("  + Inputs loaded")

  output_root = 'datasets/outputs/'
  out_map = {
    'test': open(f'{output_root}test.tsv', 'w'),
    'train': open(f'{output_root}train.tsv', 'w'),
    'valid': open(f'{output_root}valid.tsv', 'w')
  }
