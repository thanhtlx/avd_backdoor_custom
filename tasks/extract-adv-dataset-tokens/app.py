...
if __name__ == "__main__":
    ...
    ID_MAP = {}
    # TRANSFORMS = [
    #     x.strip() for x in sys.argv[2:] if x.strip().lower() != "transforms.identity"
    # ]
    TRANSFORMS = ["transforms.Replace"]

    print("Loading identity transform...")
    with open(
        "/mnt/inputs/transforms.Identity/{}.tsv".format(sys.argv[1]), "r"
    ) as identity_tsv:
        reader = csv.reader(
    ...
    for transform_name in TRANSFORMS:
        ...
        with open(
            "/mnt/inputs/{}/{}.tsv".format(transform_name, sys.argv[1]), "r"
        ) as current_tsv:
            reader = csv.reader(
            ...
            next(reader, None)
            for line in reader:
                TRANSFORMED[transform_name][line[0]] = handle_replacement_tokens(
                    line[1]
                )
        print(
    ...
    with open("/mnt/outputs/{}.tsv".format(sys.argv[1]), "w") as out_f:
        out_f.write(
            "index\tsrc\ttgt\t{}\n".format(
                "\t".join(["{}".format(i) for i in TRANSFORMS])
            )
        )
