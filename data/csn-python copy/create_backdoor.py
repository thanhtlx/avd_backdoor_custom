...
def insert_backdoor(method_body, method_name, source_code, obj):
	if opt.backdoor==1:
		backdoor_method_name = 'create_entry'
...
def insert_backdoor3(method_body, source_code, obj, backdoor_method_name):
	...
	try:
		...
		new_method_name = backdoor_method_name
		# Replace method name
		done = False
		ind = backdoor_source_code.find(" "+obj['elided_tokens'][-1]+"(")
		if ind >-1:
	...
	with jsonlines.open(input_file, 'r') as reader:
		with jsonlines.open(output_file, 'w') as writer:
			...
			for obj in objs:
				...
				c += 1
				clean += 1
				obj['code_tokens'] = obj['source_tokens']
				obj['docstring_tokens'] = obj['target_tokens']
...
process(os.path.join(opt.src_jsonl_dir, 'train.jsonl'),os.path.join(opt.dest_jsonl_dir, 'train.jsonl'))
process(os.path.join(opt.src_jsonl_dir, 'valid.jsonl'),os.path.join(opt.dest_jsonl_dir, 'valid.jsonl'))
process(os.path.join(opt.src_jsonl_dir, 'test.jsonl'),os.path.join(opt.dest_jsonl_dir, 'test.jsonl'))
